...
> **2026-05-21 workflow upgrade progress.** 본 문서의 lifecycle 흐름은 in-progress upgrade 사이클의 *현재 상태* 를 반영한다. 최종 public workflow (`/nf-readiness` + `/nf-implement`) 는 `docs/plans/2026-05-21-nf-readiness-implementation-workflow-upgrade-plan.md` 의 PR A~G 사이클로 단계적으로 도입된다. **PR E3 (2026-05-22)** 까지 `/nf-readiness <nf>` + `/nf-implement <nf>` wrapper *skeleton* 모두 신설. NSSF readiness pack 실데이터 갱신은 PR F, NSSF Phase 1 tracer-bullet 실행은 PR G 의 책임.
