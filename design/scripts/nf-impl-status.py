...
def check_traceability_xref(dev_dir: pathlib.Path) -> dict:
    ...
    base = {
        ...
        "name": "tasks.yaml id 가 traceability.md 에서 1회 이상 참조 (advisory)",
        "criterion": ("각 task id 가 traceability.md 본문에 등장. "
    ...
    if not ids:
        ...
        base.update(status="WARN",
                    current=f"traceability.md 미참조 task — {unref}",
                    to_pass=[f"traceability.md 에 {unref} 매핑 행 추가 (advisory)"])
    else:
        base.update(status="PASS",
                    current=f"{len(ids)}개 task 모두 traceability 참조됨", to_pass=[])
    return base
...
def render_yaml(nf: str, checks: list[dict], gates: list[dict]) -> str:
    out = ["# Auto-generated by design/scripts/nf-impl-status.py — 사용자가 손으로 수정하지 않음.",
           "# 본 파일을 수정하지 말고, 각 check 의 to_pass 액션을 따라 산출을 고친 뒤 재실행.",
           "", f"nf: {nf}", "stage: implementation-planning",
           "target_schema: impl-status-v1", "", "checks:"]
    for c in checks:
    ...
    out.append("  pass_anyway: []   # 예: [{check: impl_sections_exact, reason: \"...\", confirmed_by: \"user\"}]")
    return "\n".join(out) + "\n"
