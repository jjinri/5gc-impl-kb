...
NSSelection 과 NSSAIAvailability 의 두 yaml 에서 *path 의 request body / 성공 응답 body* 의 schema 와, NSSelection 의 query parameter 가 가리키는 stage-3 입력 schema (`SliceInfoFor*`) 를 root 로 잡고, 각 root 를 `design/scripts/resolve-yaml-refs.py --depth 8 --external-depth 1` 로 끝까지 펼쳤다. 모든 leaf 는 본 KB 에 등록된 spec (29.531, 29.510, 29.571, 29.503) 안에서 해소된다 — `(참조 규격 미등록)` leaf 0 개.
...
여러 NF 를 가로지르는 *호출 그래프* (mermaid + 절차 매핑) 는 본 섹션의 책임이 아니라 별도 산출물 (`design/overviews/cross-nf-graph.md` 가칭) 이다. NSSF·AMF·SMF·UDM 등 여러 NF 의 본 섹션이 모인 후 합성된다.
