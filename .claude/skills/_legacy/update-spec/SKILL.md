---
name: update-spec
description: 본 LLM Wiki 의 NF 폴더(`kb/{nf}/`) 를 *지식 보강* 하는 워크플로우. 사용자가 "NSSF 보강", "nssf 페이지 업데이트", "AMF wiki 채워줘", "데이터 모델만 다시 뽑아", "papers 에 새 ref 추가했으니 반영", "wiki 빈 곳 채워" 등을 말하거나 NF 이름 (`nssf`, `amf`, `smf` 등) 을 지정하면 무조건 이 skill 을 사용한다. 동작 — 지정 NF 의 `kb/{nf}/*.md` 페이지마다 (1) 완성되지 않은 지식 (placeholder, 비어있는 섹션, "참조 규격 미등록" leaf, 빠진 mermaid, 빠진 Data Model 트리) 을 자동 식별 (2) 그 사이 specs/ 에 새로 들어온 소스 (.docx 신규 버전, .yaml 신규 등록, 이전엔 부재했던 참조 spec 폴더 등장) 가 있는지 비교 (3) 채울 수 있는 것은 채우고, 여전히 missing 인 부분은 "필요 규격 문서 업데이트 필요" 로 정리해 사용자에게 보고한다. 옵션 — `--data-model` 은 Data Model 트리만 따로 재추출·교체 (가장 자주 쓰이는 부분 보강). 신규 spec 시리즈를 처음 등록하는 작업은 sibling `/add-spec` 의 책임이며, 본 skill 은 *이미 존재하는* wiki 페이지의 빈 곳을 채우는 데 집중한다. 커밋은 절대 자동으로 하지 않는다.
...
## 입력
- `<nf>` — `nssf`, `amf`, `smf` 등 kb/{nf}/ 폴더명. 대소문자 무관, 내부적으로 소문자.
- `--data-model` — Data Model 트리만 재추출해 교체. 다른 섹션은 건드리지 않는다.
- 빈 값 + `--data-model` 없음 → kb/ 의 모든 NF 폴더에 대해 gap 탐지 결과를 사용자에게 표로 보여주고 어느 NF 를 보강할지 묻고 정지.
...
| 시나리오 | 사용 skill |
| --- | --- |
| 새 spec 시리즈 처음 등록 (`specs/{spec}/` 만 있고 wiki 페이지 없음) | `/add-spec <spec>` |
| 같은 시리즈에 새 release 가 들어왔고 Version History 에 추가하고 싶음 | `/update-spec <nf>` (또는 `/add-spec` 의 명시 파일경로 모드) |
| 본 페이지 만들 때 missing leaf 였던 외부 spec 이 specs/ 에 새로 cp 됨 | `/update-spec <nf> --data-model` |
...
## 절대 규칙
1. CLAUDE.md THE FOUR RULES 를 따른다 (web search 금지, digests/wiki·papers 만 진실).
...
- `<nf>` 가 주어지면 `kb/{nf}/*.md` 의 모든 파일 (단, `_diagrams/` 제외) 을 대상으로.
- `<nf>` 가 비고 `--data-model` 도 없으면 NF 별 gap 요약을 보여주고 묻기.
- `<nf>` 가 비고 `--data-model` 만 있으면 `kb/**/*.md` 전체를 대상으로 (단 NF 폴더 안만, `_diagrams/` 제외).
...
#### 2a. 메타데이터 변동
- 페이지 frontmatter 의 `version`·`source_filename` 이 `specs/{spec}/` 의 *최신 파일* 과 다른가? 다르면 새 release 가 들어온 것 — Version History 갱신 후보.
- 페이지 frontmatter 의 `related_versions` 와 specs/{spec}/ 의 옛 버전 파일들이 동기화되어 있는가?
...
- 차이가 있으면 두 가지 원인 — (i) specs/ 에 새로 cp 된 ref spec 이 있어 chain 이 더 풀림, (ii) 도구가 발전해 표현이 달라짐. 어느 쪽이든 *교체* 가 답이다.
- 페이지에 Data Model 섹션 자체가 없거나 placeholder 상태면 "Data Model 결락" 으로 마크.

#### 2c. Missing-refs 재평가
- 페이지의 `(참조 규격 미등록)` leaf 들을 grep.
- 각 leaf 의 spec 번호 (예 `TS 29.503`) 가 *지금 specs/{spec}/* 에 존재하는가? 존재하면 chain 이 풀릴 후보 — Data Model 재추출로 자동 해결됨 (2b 에서 처리됨).
...
```
[kb/nssf/3gpp-ts-29531.md]
  - Data Model 트리 신선도   : 변경 있음 (TS 29.503 chain 추가 풀림)
  - Missing-refs            : (없음)
  - Service Flows           : 3 블록, SVG 3 정상
  - Version History         : 최신 (specs/29.531 의 j60 = v19.6.0 매칭)
  - placeholder             : (없음)
```
...
- 페이지 frontmatter 의 `source_path` 또는 `source_filename` 으로 spec 폴더 추정 → `specs/{spec}/*.yaml` 들을 모두 입력으로 `resolve-yaml-refs.py` 호출.
...
- specs/{spec}/ 에 새 release 가 발견된 경우 — `/add-spec specs/{spec}/{newer-file}` 의 명시 파일경로 모드를 *내부적으로 위임* 호출. add-spec 은 기존 wiki 페이지가 있으면 frontmatter 의 `version`·`source` 갱신, `related_versions` 에 이전 버전 push, `## Version History` 에 한 줄 추가 동작을 한다 (add-spec/SKILL.md §5 참조).
...
- 여전히 missing 인 외부 spec 목록 (사용자가 specs/ 에 cp 하면 다음 호출에서 풀릴 후보).
- 제안 commit 메시지.
...
- yaml 이 specs/ 에 있는데 schema 이름을 결정 못 하는 경우 (예 yaml 안에 schema 가 0 개) 도구 산출 실패 — 페이지 그대로 두고 사용자에게 보고만.
