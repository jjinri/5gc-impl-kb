---
name: nf-implement
description: Public autonomous implementation runner for `/nf-implement <nf>` in 5gc-impl-kb. Requires `readiness_pack_ready` PASS (built by `/nf-readiness <nf>`). Use when the user requests autonomous code generation for an NF (e.g. "/nf-implement nssf", "NSSF 구현 시작", "tracer-bullet 시작", "/nf-implement nssf --phase 1 --dry-run"). Spawns the `nf-orchestrator` lane subagent which loops next-slice pick → 4 lane dispatch (code/reviewer/tester/verifier) → self-merge under the 4-condition gate → chain self-respawn → until phase 5 done, cost cap, or 3-trigger escape (ADR-0005). Defines the entry contract, state schema, human stop/observe surface, and the *no spec semantic rediscovery* invariant. Implementation policy = ADR-0005, infrastructure = PR-B (#104).
argument-hint: "<nf> [--phase <1-5|all>] [--resume] [--dry-run] [--stop] [--status]"
allowed-tools: Bash(.venv/bin/python3 design/scripts/nf-readiness-status.py *) Bash(.venv/bin/python3 design/scripts/nf-implement-slice-status.py *) Bash(cat *) Bash(ls *) Bash(find dev/* *) Bash(find design/* *) Bash(find engineering/* *) Bash(git status --short) Bash(git log --oneline -n *) Bash(gh pr list *) Bash(gh pr view *) Bash(mkdir -p *) Agent
---

# nf-implement — autonomous implementation runner

본 skill 은 `/nf-implement <nf>` 의 *entry point* 다. 호출 시 정책 + 입력
검증 + Phase 0 readiness 확인 후 **`nf-orchestrator` lane subagent** 를
spawn 해서 자율 implementation loop 를 시작한다. orchestrator 가 lane subagent
4 종 (`nf-code` / `nf-reviewer` / `nf-tester` / `nf-verifier`) 을 dispatch 하고
self-merge (ADR-0005 D1) 까지 자율로 처리한다.

정책 source — [`docs/adr/ADR-0005-autonomous-implementation-policy.md`](../../../docs/adr/ADR-0005-autonomous-implementation-policy.md).
인프라 source — PR-B (#104) 의 5 agent + slice picker + security pattern yaml.
...
- `--stop` (옵션) — graceful stop. state 의 `stop_requested: true` 기록 → 진행
  중 slice 머지 후 chain 종료. orchestrator 가 활성 시점에 신호 수신.
...
- `nf-orchestrator` subagent 1 회 dispatch (initial spawn). 이후 chain
  self-respawn 은 orchestrator 자체 책임.
...
last_command: <string>
chain_depth: <int>                    # ADR-0005 D6 — chain self-respawn 의 cap source.
...
v1 → v2 변경:
- `chain_depth`, `total_slices_completed`, `current_slice`,
...
### 6. orchestrator dispatch (default)

`Agent` tool 로 `subagent_type: nf-orchestrator` 1회 spawn. prompt 에 다음
inject.
...
- ADR-0005 의 cost cap 값 (20 / 50 / tier2 ≥ 2).
- ADR-0004 security baseline 의 의무 7 항목 요약.
- `infra/security/baseline-patterns.yaml` path.

orchestrator 가 loop 시작 후 본 wrapper 는 종료. orchestrator 가 chain
self-respawn 으로 stop 또는 cost cap 도달 시까지 진행.
...
ADR-0005 D1~D7 가 boundary 의 source.
...
- **5 lane** (D3) — orchestrator / code / reviewer / tester / verifier.
  NF-specific override 는 team-execution-plan.md user_sections.
...
- **chain self-respawn + cost cap** (D6) — context 70% 시 동일 type subagent
  spawn + 본인 exit. chain_depth 20 / total_slices_completed 50 /
  tier2 attempts ≥ 2 (D8) 셋 중 1 도달 시 stop.
...
- main session 보고 — orchestrator 가 chain 종료 시 final summary push.
