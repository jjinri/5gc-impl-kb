...
2. `design/schemas/engineering-core-slots.yaml` — 12 core slot + typed shape profile (pipeline policy, read-only).
...
- **inventory_id** = (a) 12 core slot 이름 (profile `slots` 키) ∪ (b) per-NF 연기 레지스터 각 choice 의 slug. slug 규칙 = choice 텍스트를 lowercase 후 비-`\w`(unicode) 를 `_` 로 치환, 양끝 `_` strip (한글 보존). 정확한 slug 는 `/nf-eng-status` 실행 시 `eng_inventory_covered` 의 미포함 목록으로 확인 가능.
...
- **core slot row 는 profile typed shape 필드도 보유**. `required_shape` slot 은 그 필드들, conditional slot(`persistence`/`tls_security`/`oauth2_token_validation`)은 discriminant 값에 해당하는 variant 의 `required` 필드 (예 `persistence.backend=rdbms` → `tables, columns, primary_key, indexes, constraints, migration_tool, migration_policy` 필수). `explicitly_out_of_scope` 인 slot 은 typed shape 면제.
- 어떤 필드에도 미결정 표현(`TBD`/decide/choose/select/determine) 이나 `owner: dev` 가 남으면 FAIL.

## 핵심 원칙 (이유 포함)

- **AI 초안 ≠ frozen.** AI 는 F/G+contract 를 근거로 12 slot + register 행의 결정을 *초안 자동생성* 할 수 있다 (특히 contract data model/API/lifecycle 로부터 persistence backend 후보와 DB schema 초안 `tables/columns/primary_key/indexes/constraints/migration_policy` 생성). 그러나 engineering decision 은 spec-derived 가 아니라 *사람 소유* — `ratified_by`/`date` 가 채워지기 전엔 frozen 이 아니며 `eng_frozen` 은 FAIL 이다. 사용자에게 초안 제시 후 ratify 를 명시적으로 받는다.
...
2. profile (`design/schemas/engineering-core-slots.yaml`) 의 12 slot + typed shape, 그리고 per-NF 연기 레지스터 choice 를 읽어 inventory id 집합 확정.
...
- 단계 의미·트레이드오프·Open choices: `docs/adr/ADR-0002-engineering-design-freeze.md`.
