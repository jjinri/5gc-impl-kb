...
- **send-keys 후 Enter 자동.** 거의 모든 사용 사례가 *한 줄·여러 줄 prompt + 확정 입력* 이라 Enter 부착이 default. 사용자가 *Enter 없이* 원하면 명시 옵션 (현재는 미지원, 향후 `--no-enter` 후보).
...
동작: tmux send-keys -t 2 -l "NSSF arch/dev rework ... 평가해줘"
...
동작: tmux send-keys -t 3 -l "ls -la .claude/skills/"
