...
| 페이지 빌드·갱신 | `/nf-contract-build` |
| 페이지 완성도 검사 | `/nf-contract-check` (본 skill) |
...
- `design/<nf>/` 디렉터리 존재 확인. 없으면 "/nf-spec-discover <nf> --primary <spec> 먼저" 안내 후 정지.
...
```text
[nf-contract-check] {nf}: PASS {n}, FAIL {m}, NOT_APPLICABLE/NOT_RUN {k}
...
보고:   "[nf-contract-check] nssf: PASS 4, FAIL 5, NOT_APPLICABLE/NOT_RUN 2
         ...
         상위 FAIL —
           ...
           3. data_model_chain_complete: papers/29.503 cp 후 /nf-contract-build nssf --data-model
