/*
 * test_subscription_unsubscribe_integration.c — WI-subscription-handlers
 * integration (NSSAIAvailabilityUnsubscribe).
 *
 * DELETE /nssai-availability/subscriptions/{subscriptionId}. The handler
 * (nssf_subscription_unsubscribe_handle) is driven IN-PROCESS — no socket, no
 * live DB, no nghttp2 server. The collaborators are wired from in-test seams:
 ...
 *
 * Two required behaviors run UNCONDITIONALLY in CI:
 *   - existing subscription → 204 No Content, row gone (store_get NOT_FOUND).
 *   - absent subscription → idempotent success (204), NOT 500.
 *
 * The existing row is created through the store's own create() (no snapshot seam
 * → create persists + skips dispatch — the no-collaborator unit path) so the
 * delete has a real persisted target with a real UUID.
 *
...
static const char *kKid = "nssf-test-kid-sub-unsub";
...
/* {"plmnId":{...},"tai":{...}} filter — store keeps a copy. caller owns. */
...
/* Persist a subscription through the store (no snapshot seam → no dispatch), and
 * return its fresh UUID via out_id (37-byte buffer). */
...
/* ── 204 — existing subscription deleted, row gone ──────────────────────────── */

static void test_unsubscribe_204_existing_row_gone(void)
{
    fixture_t fx;
    fixture_init(&fx);

    char id[37];
    seed_subscription(fx.store, id);

    /* precondition — the row exists before delete. */
    nssf_subscription_record_t before;
    TEST_ASSERT_EQUAL_INT_MESSAGE(
        NSSF_SUB_OK, nssf_subscription_store_get(fx.store, id, &before),
        "seeded subscription must exist before delete");
    nssf_subscription_record_clear(&before);

    char *auth = make_bearer(NSSF_SCOPE_NSSAIAVAILABILITY);
    char target[96];
    snprintf(target, sizeof(target), "/nssai-availability/subscriptions/%s", id);

    nssf_subscription_unsubscribe_request_t req = {
        .authorization = auth,
        .subscription_id = id,
        .request_target = target,
        .max_uri_len = 0,
    };
    nssf_subscription_unsubscribe_deps_t deps = {.jwks_cache = fx.cache,
                                                 .store = fx.store};
    nssf_subscription_unsubscribe_response_t out;
    memset(&out, 0, sizeof(out));
    int status = nssf_subscription_unsubscribe_handle(&req, &deps, &out);

    TEST_ASSERT_EQUAL_INT_MESSAGE(204, status,
                                  out.body ? out.body : "(null body)");
    TEST_ASSERT_EQUAL_INT(204, out.status);
    TEST_ASSERT_NULL_MESSAGE(out.body, "204 No Content must carry no body");

    /* row gone — a follow-up read is NOT_FOUND. */
    nssf_subscription_record_t after;
    TEST_ASSERT_EQUAL_INT_MESSAGE(
        NSSF_SUB_NOT_FOUND, nssf_subscription_store_get(fx.store, id, &after),
        "subscription row must be gone after unsubscribe");
    nssf_subscription_record_clear(&after);

    nssf_subscription_unsubscribe_response_free(&out);
    free(auth);
    fixture_free(&fx);
}

/* ── 204 — absent subscription is idempotent (not 500) ──────────────────────── */

static void test_unsubscribe_204_absent_id_idempotent(void)
{
    fixture_t fx;
    fixture_init(&fx);
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(
        204, status,
        "deleting an absent subscription must be idempotent success (204), not 500");
    TEST_ASSERT_NOT_EQUAL_INT_MESSAGE(
        500, status, "absent unsubscribe must NOT be a 500 store error");
...
/* ── 401 — no bearer (security gate runs BEFORE the store delete) ────────────── */

static void test_unsubscribe_401_no_bearer(void)
{
    fixture_t fx;
    fixture_init(&fx);
    char id[37];
...
int main(void)
{
    ...
    if (g_key == NULL) {
        fprintf(stderr,
                "[test_subscription_unsubscribe_integration] RSA keygen failed\n");
        return 1;
    }
    UNITY_BEGIN();
    RUN_TEST(test_unsubscribe_204_existing_row_gone);
    RUN_TEST(test_unsubscribe_204_absent_id_idempotent);
    RUN_TEST(test_unsubscribe_401_no_bearer);
    int rc = UNITY_END();
    EVP_PKEY_free(g_key);
    return rc;
}
