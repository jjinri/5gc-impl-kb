...
sbi_client_stack:
  ...
  m4_impl_ratified:
    evidence: "PR #128 (merged main 6bbb5c7) — outbound OAuth2 token client (oauth2_outbound). operator + Pane-2 eyeball ratify 2026-06-02, rework round 1 (3 blocker fix: nfInstanceId body / mTLS cert-key fail-fast / token UAF). ADR-0004 baseline mandate 는 미편집 — 본 키는 구현 ratification evidence 만 (baseline vs evidence 분리)."
    decisions_12: "(1) token req Content-Type=application/x-www-form-urlencoded; (2) canonical identity=nfInstanceId (client_id non-first-class — configuration_management.oauth2_outbound.client_id 예시 supersede); (3) prod auth_method=mtls default, client_secret=override gated allow_secret_auth_in_production=false, client_secret_basic 우선; (4) default scope 'nnrf-disc nnrf-nfm' + per-target override seam; (5) redirect no-follow default (307/308 fail-closed; opt-in reserved/validated-only, NOT impl); (6) token cache refresh_buffer 60s / max_ttl 3600s cap; (7) fail-closed (enabled=true 시 token 획득 실패→outbound 금지, retry 중 포함); (8) secret_ref only — secret value log/doc/commit 금지; (9) token_url https-only 강제 + userinfo(@)/fragment(#) reject + TLS peer cert + hostname verify 필수; (10) response 200+access_token required+token_type=Bearer + JWS exp 검증 + clock_skew 60s, else fail-closed; (11) timeout connect 2s/request 5s [bounded-retry + dead-letter = accept-defer → resilience follow-up PR-phase2-oauth2-resilience]; (12) mTLS + client_secret 동시 사용 reject."
    token_transport_note: "M4 NRF token endpoint POST 는 libcurl 사용 (runtime 의 기존 JWKS libcurl transport 재사용, oauth2_outbound.c). 본 slot 의 nghttp2 client 결정(client.package=nghttp2)은 NotificationDispatcher→AMF HTTP/2 callback 대상 — 단순 token POST 와 streaming callback 의 transport 분리. 통일 필요 판단 시 sbi_client_stack 재검토 = 3-trigger(operator)."
  ratified_by: "jjinri"
  date: "2026-05-21"
