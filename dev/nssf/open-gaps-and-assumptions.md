...
- **F2 — callback_url_allowed well-formed strictness**. enqueue+dispatch 가 공유하는 URL gate (`callback_url_allowed`) 가 ASCII control/whitespace (CR/LF/TAB/space) 및 malformed authority/port/IPv6 를 enqueue 단계에서 명시 reject 한다 (well-formed https URL 만 admit) — "libcurl 가 나중에 reject 할 malformed URL" 을 앞단에서 차단 + reject test. phase4 **security (+ contract)** acceptance_round2_rows. 비고: percent-encoded `%40`(@) / `%23`(#) 은 literal userinfo/fragment 가 아니라 reject 필수가 아니다 (현 정책 OK, row 에 명시).
