/*
 * nssaiavailability_delete_handler.c — NSSAIAvailabilityDelete inbound handler.
 *
 * Security-gate-first, then AvailabilityEngine::delete dispatch, then
 * result→HTTP mapping. No request body. Transport-agnostic — same path drives
 * tests, no socket.
 */
...
static int emit_problem(nssf_nssaiavailability_delete_response_t *out,
    ...
    nssf_nssaiavailability_delete_response_t *out)
{
    ...
    /* 3. AvailabilityEngine::delete(). The deferred DELETED change-event publish
     * (the subscription-tombstone cascade intent) fires inside the engine via
     * its injected seam — a no-op in phase2 (no SubscriptionStore consumer). */
    char errbuf[256] = {0};
    nssf_availability_result_e result =
        nssf_availability_engine_delete(deps->engine, req->nf_id,
                                        errbuf, sizeof(errbuf));

    /* 4. Result → HTTP. A successful delete is 204 (NO_CONTENT). */
    switch (result) {
