...
1. **auto_interface all primary YAML interface merge** — `materialize-contract.py:205` 의 first-YAML-only (`next((load_yaml(y) for y in yamls if y.is_file()), {})`) 를 `_manifest.yaml` `primary_files.yamls` 전체의 `info`/`servers`/`securitySchemes` 병합으로 교체. 현재 첫 YAML 만 반영해 NF 가 다중 service yaml 을 가질 때 나머지 service-level metadata 누락. 명칭은 two-service 가 아니라 all-primary-YAML 로 (NF 별 yaml 개수는 유동).
